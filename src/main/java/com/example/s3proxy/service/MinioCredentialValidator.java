...
public class MinioCredentialValidator {
    ...
    public boolean validateAccessKeyExists(String accessKey) {
        ...
        log.info("Validating access key exists: {} against endpoint: {}", accessKey, minioEndpoint);
        try {
            MinioClient tempClient = MinioClient.builder()
                    .endpoint(minioEndpoint)
            ...
            log.info("MinIO server accessible for access key: {}", accessKey);
            return true;
            
        } catch (Exception e) {
            // Check if the error indicates server unavailability vs auth failure
            String message = e.getMessage().toLowerCase();
            log.info("MinIO validation error for access key {}: {}", accessKey, e.getMessage());
            if (message.contains("connection refused") || message.contains("connection timeout") ||
                ...
                log.info("MinIO server not accessible: {}", e.getMessage());
                ...
                log.info("MinIO server accessible, auth error expected for dummy credentials: {}", e.getMessage());
                return true;
            }
        }
    }
